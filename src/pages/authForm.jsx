...
const AuthForm = ({ roles }) => {
  ...
  async function handleSubmit(event) {
    ...
    let blbl = await axios
      .get(`http://localhost:8000/show/card/`)
      .then((res) => {
        return res.data;
      });
  ...
  return (
    <div>
      <div className="container__wrapper d-flex-center-center">
        <form>
          <input
            className="inpt1"
            value={login}
          ...
          />
          <button type="submit" className="authBtn w-100 d-flex" onClick={handleSubmit}>
            готово
          </button>
        </form>
      </div>
